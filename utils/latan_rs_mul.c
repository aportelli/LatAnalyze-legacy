...
int main(int argc, char* argv[])
{
	...
	res = rs_sample_create(s1_nrow,s1_nsample,OUTF_NAME);
	...
	/* multiplying samples */
	printf("-- multiplying samples...\n");
	mat_mulp(rs_sample_pt_cent_val(res),rs_sample_pt_cent_val(s1),\
			rs_sample_pt_cent_val(s2));
	for (i=0;i<rs_sample_get_nsample(res);i++)
	{
		mat_mulp(rs_sample_pt_sample(res,i),rs_sample_pt_sample(s1,i),\
